...
describe('GET /api/v1/products', () => {
  ...
  it('lists products with default pagination metadata', async () => {
    ...
    const body = asSuccessBody<Product[], ProductListMeta>(response.body);
    ...
    expect(body.data.map((product) => product.name)).toEqual(
      expect.arrayContaining(['Wireless Mouse', 'Mechanical Keyboard', 'USB-C Hub'])
    );
  });
  ...
  const seedProducts = async (products: Array<[string, string, number, number]>) =>
    database.models.Product.bulkCreate(
      products.map(([name, description, price, stockQuantity]) => ({
        name,
        description,
        price,
        stockQuantity
